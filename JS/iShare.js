...
(function (root, factory) {
  ...
  //alert("aaa1");
  //alert(this.defaults.indexOf('iShare_wechat'));
...
//alert("aaa3");
    ...
    function init(obj) {
        //alert(9);
        if (obj.settings.initialized) {
            //alert(2);
            //_autoUpdate.call(obj);
            _autoUpdate(obj)
        } else {
           // alert(3);
            _updateUrl(obj);
        ...
        if (obj.wx) {
            ...
            obj.wx.init();
            // 加载qrcode库
            Util.loadjs('qrcode.min.js', obj.wx.startQR());
        }
    }
    ...
    * @return {DOMObject}           
    */
    function _createShareElements(url, item, isWechat, obj) {
        ...
        if (isWechat) {
            this.wx = new WX(_e, url, obj.settings.WXoptions);
    ...
    * _autoUpdate 动态创建分享
    */
    function _autoUpdate(obj) {
        var _docfrag = document.createDocumentFragment(),
					_tpls = Util.parseUrl(_templates, obj.settings),
					_element,
					_me = obj;
        ...
        Util.each(_me.settings.sites, function (index, item) {
            if (item === 'iShare_wechat') {
                ...
                _element = _createShareElements.call(_me, _me.settings.url, item, true, obj);
            } else {
                _element = _createShareElements.call(_me, _tpls[item], item, false, obj);
            }
            _docfrag.appendChild(_element);
        });
        obj.container.innerHTML = '';
        obj.container.appendChild(_docfrag);
    }
    ...
    */
    function _updateUrl(obj) {
        if (!obj.container.hasChildNodes()) {
            return;
        }
        var _children = obj.container.childNodes,
					_tempURL;

        for (var i = 0, item; item = _children[i++]; ) {
            if (item.nodeType === 1) {
                _tempURL = Util.parseClassName(item.className, Util.parseUrl(obj._templates, obj.settings));
                if ((item.className).indexOf('iShare_wechat') > -1) {
                    // this.wx = new WX(item, _tempURL, this.settings.WXoptions);
                    obj.wx = new WX(item, obj.settings.url, obj.settings.WXoptions);
                } else {
