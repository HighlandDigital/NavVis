...
namespace DAO
{
    ...
    public class DAO_industry
    {
        ...
        public bool DeleteByID(int id)
        {
            ...
            strSql.Append("delete from obj_industry");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * from obj_industry ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
                new SqlParameter("@id", SqlDbType.Int)
          };
        ...
        public DataTable GetList(int pageNum, int pageSize, string name, string description, string orderBy, out int count)
        {
            ...
            parameters[0].Value = "%" + name + "%";
            parameters[1].Value = "%" + description + "%";
            sqlCount.Append("select id from obj_industry where 1=1 ");
            sqlStr.Append("select top " + pageSize + " * from obj_industry where 1=1 ");//用于查询返回当前页数据
            if (name != "")
            {
            ...
            if (pageNum > 1)
            {
                sqlStr.Append("and id not in(select top " + (pageNum - 1) * pageSize + " id from obj_industry where 1=1");
                sqlStr.Append(sqlWhere);
                if (orderBy != "")
                {
